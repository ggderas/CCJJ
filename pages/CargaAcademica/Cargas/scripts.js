...
function request(input){
  ...
  var params = "cod_periodo="+cod_periodo+"&cod_estado="+cod_estado+
               "&no_empleado="+no_empleado+"&anio="+anio;
  ...
  if(input == "guardar" && listoParaPeticion(cod_periodo,cod_estado,no_empleado,anio)){
...
function listoParaPeticion(cod_periodo,cod_estado,no_empleado,anio){
  if(no_empleado == 0){
    document.getElementById("alerta").className = "alert alert-danger alert-error";
    ...
    document.getElementById("alerta").className = "alert alert-danger alert-error";
    document.getElementById("alerta").innerHTML = "<strong>¡Error! </strong>Debe ingresar el Año.";
...
$(document).on("click",".editar",function () {
  ...
  var anio = $(this).parents("tr").find("td").eq(3).attr('id');
  var estado = $(this).parents("tr").find("td").eq(4).attr('id');
